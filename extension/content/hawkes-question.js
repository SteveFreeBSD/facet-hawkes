...
(() => {
  ...
  const primaryQuestionOwners = ["questionString", "questionDescription", "partDescription"]
    .map((id) => document.getElementById?.(id))
    .filter(Boolean);
  const allMath = [...document.querySelectorAll("math")];
  // `questionString` owns the main displayed expression. `questionDescription`
  // can also own inline mathematical data that changes which answer is
  // requested -- notably a coordinate pair that a constructed line must pass
  // through. Taking only the first non-empty owner silently discarded that
  // point whenever a separate equation existed.
  //
  // Do not merge arbitrary description math into a populated questionString:
  // Hawkes also renders explanatory forms such as Ax+By<C there. A coordinate
  // pair has an exact structural signature in MathML (one comma, enclosing
  // parentheses, and no relation), so it can be retained without guessing
  // from prose or reopening those templates as stated equations.
  const ownedMath = (owner) => allMath.filter((math) => owner?.contains?.(math));
  const questionMath = ownedMath(document.getElementById?.("questionString"));
  const descriptionMath = ["questionDescription", "partDescription"]
    .flatMap((id) => ownedMath(document.getElementById?.(id)));
  const coordinateMath = (math) => {
    const operators = [...(math.querySelectorAll?.("mo") ?? [])]
      .map((operator) => String(operator.textContent ?? "").trim());
    return operators.filter((operator) => operator === ",").length === 1
      && operators.some((operator) => operator === "(")
      && operators.some((operator) => operator === ")")
      && !operators.some((operator) => ["<", "≤", ">", "≥", "="].includes(operator));
  };
  const primaryMath = questionMath.length > 0
    ? [...questionMath, ...descriptionMath.filter(coordinateMath)]
    : descriptionMath;
  // When Hawkes publishes math in its dedicated question owner, that is the
  // exact source. `partInformation` may also contain the whole answer template
  // and therefore MathJax generated by a partially drawn response.
  const mathCandidates = primaryMath.length > 0 ? primaryMath : allMath;
  const visibleAnswerControls = [...document.querySelectorAll(ANSWER_CONTROLS)]
    .filter(visible);
  const answerOwns = (math) => {
    let ancestor = math.parentElement;
    for (let depth = 0; ancestor && depth < 10; depth += 1) {
      const marker = `${ancestor.id ?? ""} ${ancestor.className ?? ""}`;
      if (/answer|graph/i.test(marker)
        && visibleAnswerControls.some((control) => ancestor.contains?.(control))) {
        return true;
      }
  ...
  const mathNodes = [];
  for (const math of mathCandidates) {
    if (!visible(math) || math.getBoundingClientRect().top >= limit) {
      continue;
    }
    if (answerOwns(math)) continue;
    // Once Hawkes publishes its canonical question containers, mathematics
    // generated by a partially populated answer graph is not question math,
    // even when that graph happens to render above the first coordinate box.
    // Pages without those containers retain the geometry fallback used by
    // older fixtures and question families.
