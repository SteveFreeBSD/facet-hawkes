...
(() => {
  ...
  const HAWKES_READER_BUILD = "1f4359e1e425";
  ...
  const primaryQuestionOwners = ["questionDescription", "questionString"]
    .map((id) => document.getElementById?.(id))
    .filter(Boolean);
  const allMath = [...document.querySelectorAll("math")];
  const primaryMath = allMath.filter((math) =>
    primaryQuestionOwners.some((owner) => owner.contains?.(math)));
  // When Hawkes publishes math in its dedicated question header, that is the
