...
(() => {
  ...
  const primaryQuestionOwners = ["questionString", "questionDescription"]
    .map((id) => document.getElementById?.(id))
    .filter(Boolean);
  const allMath = [...document.querySelectorAll("math")];
  // `questionString` is Hawkes' owner for the stated expression.  The prose
  // `questionDescription` can also contain explanatory or template MathJax;
  // use it only on pages where the expression owner has no mathematics.
  const primaryMath = primaryQuestionOwners.map((owner) =>
    allMath.filter((math) => owner.contains?.(math)))
    .find((owned) => owned.length > 0) ?? [];
