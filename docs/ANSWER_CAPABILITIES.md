...
| two affine inequalities joined by their exact logical connector | `inequality-pair` | plain | `-6<2*x + 10 AND 2*x + 10<6` | two owned expression editors plus one semantic AND/OR group | `answer_payload` → `enterPlan` | yes | no | no | `absolute-value-inequality-rewrite` |
