...
| literal points graph plan | `point_plot_plan` | *plan* | `plan, no value` | one draggable control per point; labeled requests bind each label/pair to a unique XML/model point ID and its named anchor | `graphOperation` | yes | yes | yes | `plan-point-plot` |
