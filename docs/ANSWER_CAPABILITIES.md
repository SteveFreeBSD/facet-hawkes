...
| two affine inequalities joined by their exact logical connector | `inequality-pair` | plain | `2x+10>-6 AND 2x+10<6` | two owned expression editors plus one semantic AND/OR group | `answer_payload` → `enterPlan` | yes | no | yes | `absolute-value-inequality-rewrite` |
