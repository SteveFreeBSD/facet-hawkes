...
`parts`, `choice`, `conditional-choice`, `relation` -- beside `entry_mode`. It is deliberately the
...
| axis intercepts, including a semantically absent intercept | `axis-intercepts` | plain | `0` | two coordinate-pair rows, each with its own absent radio | `axisInterceptsOf` → `selectAxisAbsences` → `enterOwnedFields` → `verifyAxisInterceptInsertion` | yes | no | yes | `axis-intercepts-plain` |
| axis intercepts with a rational coordinate | `axis-intercepts` | fraction | `1/2` | two coordinate-pair rows whose boxes reveal denominator controls, each with its own absent radio | `axisInterceptsOf` → `selectAxisAbsences` → `enterOwnedFields` → `verifyAxisInterceptInsertion` | yes | no | no | `axis-intercepts-fraction` |
...
| one rendered graph chosen by exact boundary and shaded-region semantics | `choice` | plain | `Graph: x=-4 dashed; x=3 dashed; shade=between` | several complete Cartesian SVGs, each joined to one QADispatcher model and one role=radio owner | `answerFitsEditor` → `graphOperation` | yes | no | yes | `absolute-value-inequality-graph-choice` |
...
| named alternative with a conditionally enabled equation | `conditional-choice` | plain | `9x-4y=0` | a radio group whose Linear choice enables one standard-form equation box | `answerForSurface` → `planEntry` → `enterPlan` | yes | no | no | `conditional-choice-plain` |
...
| points graph plan | `point_plot_plan` | *plan* | `plan, no value` | one draggable control per point; stated points read literally, labeled requests bind label/pair to a unique XML/model point ID; derived integer-line-points find two distinct in-bounds integer solutions of the equation exactly, with independent equation/integer/bounds/snap proof on both sides | `graphOperation` | yes | yes | yes | `plan-point-plot` |
...
| exact linear inequality system graph plan | `linear_inequality_system_graph_plan` | *plan* | `plan, no value` | two mounted boundaries, a union/intersection choice, and page-owned combined regions | `graphOperation` | yes | no | no | `plan-linear-inequality-system-graph` |
