...
| one rendered graph chosen by exact boundary and shaded-region semantics | `choice` | plain | `Graph: x=-4 dashed; x=3 dashed; shade=between` | several complete Cartesian SVGs, each joined to one QADispatcher model and one role=radio owner | `answerFitsEditor` | yes | no | yes | `absolute-value-inequality-graph-choice` |
