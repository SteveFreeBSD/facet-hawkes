...
**The page's own answer model chooses the writer; the answer never does.**
`extension/common/transport.js` holds the whole decision, takes no answer and
cannot be given one. A dynamic math editor receives every character and every
...
policy did, sending an answer whose characters happened to be typeable to a DOM
writer and everything else to the editor.

→ [Hawkes answer editor: observed behaviour](HAWKES_EDITOR_FINDINGS.md) records
the editor's own character branch and the transport table.
