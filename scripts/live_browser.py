...
def popup() -> int:
    ...
    try:
        ...
        print(json.dumps(_panel_state(marionette) or {"error": "panel is not open"}, indent=2))
    finally:
        marionette.disconnect()
    return 0
...
def shot(out_path: str = "") -> int:
    ...
    try:
        marionette.set_context("content")
        result = marionette.send("WebDriver:TakeScreenshot",
...
def _ask_host(png: bytes, prompt: str, timeout: int = 900) -> dict:
    ...
    process = subprocess.Popen(
        [str(LAUNCHER)], stdin=subprocess.PIPE,
...
def _run_page_script(marionette, path: Path, tail: str = "") -> dict:
    ...
    wrapped = "return JSON.stringify((function(){ %s })());" % (
        f"const __r = eval({json.dumps(source)}); {tail} return __r;" if not tail
...
def solve_current(insert: bool = True) -> int:
    ...
    try:
        marionette.set_context("content")
        shot = marionette.send("WebDriver:TakeScreenshot",
                               {"id": None, "full": False, "hash": False, "scroll": False})
        png = base64.b64decode(shot["value"])
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        (STATE_DIR / "question.png").write_bytes(png)
        print(f"screenshot  {len(png)} bytes -> {STATE_DIR / 'question.png'}", flush=True)
        ...
        print(f"ethnos      {time.monotonic() - started:.0f}s  status={reply.get('status')}",
              flush=True)
        ...
        print(f"answer      display={answer_payload.get('display_text')!r} "
              f"keys={answer_payload.get('keyboard_entry')!r}")
        print(f"certainty   source={certainty.get('source')} "
        ...
        for candidate in (answer_payload.get("display_text"),
        ...
        outcome = _run_page_script(
            marionette, EDITOR_JS,
            tail=f"return ethnosHawkes.insertAnswer({json.dumps(chosen)});",
        )
        time.sleep(1.2)
        state = json.loads(marionette.execute(r"""
...
def insert() -> int:
    ...
    try:
        ...
        """ % PANEL_VIEW)
        time.sleep(2.5)
        print("panel after insert:", json.dumps(_panel_state(marionette), indent=2), flush=True)
    finally:
        marionette.disconnect()
    return 0
...
COMMANDS = {"start": start, "popup": popup, "page": page,
            "field": field, "watch": watch, "insert": insert,
            "shot": shot, "solve": solve_current, "stop": stop}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in COMMANDS:
        print(f"usage: {sys.argv[0]} {{{'|'.join(COMMANDS)}|go <url>}}", file=sys.stderr)
        raise SystemExit(2)
